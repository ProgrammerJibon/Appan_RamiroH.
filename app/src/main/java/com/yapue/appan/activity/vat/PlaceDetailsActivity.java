package com.yapue.appan.activity.vat;
...
public class PlaceDetailsActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.tvUrl:
                goAddress();
                mapLocation(nearByDTO.getAddress(), nearByDTO.getLati(), nearByDTO.getLongi());
                break;
    ...
    private void goAddress() {
        try {
            if(nearByDTO.getLati()!=null || nearByDTO.getLongi()!=null){
                String uri = String.format(Locale.ENGLISH, "geo:0,0?q="+nearByDTO.getLati()+","+nearByDTO.getLongi()+"?z=10");

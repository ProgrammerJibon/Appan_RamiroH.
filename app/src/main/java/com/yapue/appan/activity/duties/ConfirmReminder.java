...
public class ConfirmReminder extends AppCompatActivity implements View.OnClickListener {
    ...
    public void setInfo() {
        ...
        try {
            dates = String.valueOf(getDateinmilliSeconds(reminder.getDate_string()));
        }catch (Exception e){
            e.printStackTrace();
        }
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Yes, Ok!", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        try{
//                            DatabaseHelper.getInstance(ConfirmReminder.this).deleteSingleAlarm(deleteReminderId());
                        }catch (Exception e){
                            e.printStackTrace();
                        }
                    }
    ...
    /*public void deleteReminder(){
        final List<AlarmDTO> alarms = DatabaseHelper.getInstance(this).getAlarms();

        for (AlarmDTO alarmDTO:alarms){
            if(dates.equals(alarmDTO.getDatetime())){
    ...
    public long getDateinmilliSeconds(String myDate){
        long millis = 0;
        try{
            SimpleDateFormat sdf = new SimpleDateFormat("EEE, MMM d, yyyy hh:mm a");
            Date date = sdf.parse(myDate);
            millis = date.getTime();
            Log.e(TAG, "time in miliseconds:-------- "+millis);
        }catch (Exception e){
            e.printStackTrace();
        }
        return millis;
    ...
    public long deleteReminderId(){
        long id=0;
        final List<AlarmDTO> alarms = DatabaseHelper.getInstance(this).getAlarms();
        for (AlarmDTO alarmDTO:alarms){
            Log.e(TAG, "datetime----- "+alarmDTO.getId());
            if(dates.equals(String.valueOf(alarmDTO.getDatetime()))){
                id=alarmDTO.getId();
            }
        }
        return id;
